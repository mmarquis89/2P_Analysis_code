...
timeLimitMin = 400;
queueName = 'short';
jobName = ['make_behavior_vids_', expDate]
...
timeLimitMin = 60;
queueName = 'short';
jobName = ['make_fluorescence_vid_', expDate]
c = set_job_params(c, queueName, timeLimitMin, memGB, jobName);
...
timeLimitMin = 30;
queueName = 'short';
jobName = ['create_anatomy_stack_', expDate]
c = set_job_params(c, queueName, timeLimitMin, memGB, jobName);
...
timeLimitMin = 240;
queueName = 'short';
jobName = ['pre_reg_processing_', expDate];
c = set_job_params(c, queueName, timeLimitMin, memGB, jobName);
...
memGB = ceil(2.13e-08 * nColumns * nLines * nPlanes * nVolumes * nTrials);
if memGB > 249
    memGB = 249;
end
timeLimitMin = ceil(7.6294e-08 * nColumns * nLines * nPlanes * nVolumes * nTrials);
if timeLimitMin > 719
    timeLimitMin = 719;
end
queueName = 'short';
jobName = ['NoRMCorre_', expDate];
c = set_job_params(c, queueName, timeLimitMin, memGB, jobName);
...
memGB = ceil(1.5259e-08 * nColumns * nLines * nTrials * nVolumes * nPlanes);
if memGB > 249
    memGB = 249;
end
timeLimitMin = ceil(7.6294e-08 * nColumns * nLines * nTrials * nVolumes * nPlanes);
if timeLimitMin > 719
    timeLimitMin = 719;
end
queueName = 'short';
jobName = ['pca_calc_', expDate];
c = set_job_params(c, queueName, timeLimitMin, memGB, jobName); 
