...
objectiveResolution = 50.7043;     % Resolution of the objective in microns/degree of scan angle

% Scanner systems
scannerNames = {'ResScanner' 'LinScanner'};     % Cell array of string names for each scan path in the microscope
scannerTypes = {'Resonant' 'Linear'};     % Cell array indicating the type of scanner for each name. Current options: {'Resonant' 'Linear' 'SLM'}
...
beamDaqs(1).calOffsets = 0.0125191;     % Array of beam calibration offset voltages for each beam calibration channel
...
motors(1).comPort = 32;     % Integer identifying COM port for controller, if using serial communication
...
motors(1).moveTimeout = 30;     % Default: 2s. Fixed time to wait for motor to complete movement before throwing a timeout error
...
actuators(1).controllerType = 'pi.e753';     % If supplied, one of {'pi.e665', 'pi.e816', 'npoint.lc40x', 'analog'}.
...
%% ResScan (ResScanner)
nominalResScanFreq = 7920.79;     % [Hz] nominal frequency of the resonant scanner
...
resonantZoomDeviceName = 'PXI1Slot4';     % String identifying the NI-DAQ board to host the resonant zoom analog output. Leave empty to use same board as specified in 'galvoDeviceName'
...
resonantEnableTerminal = 1;     % (optional) The PFI line on the resonantZoomDevice that enables/disables the resonant scanner. Example: 13   Not required for Thorlabs BSCope
...
ecuComPort = 29;     % Numeric: Serial COM port for ECU2 commands
...
galvoResonantMirrorComPort = 34;     % Numeric: Serial COM port for controlling Galvo-Resonant mirror. Leave empty if mirrors are connected to stage controller (MCM5000)
flipperMirrorComPort = 5;     % Numeric: Serial COM port for controlling Flipper mirror. Leave empty is mirrors are connected to stage controller (MCM5000)
...
scanParkAngleX = -10;     % Numeric [deg]: Optical degrees from center position for X galvo to park at when scanning is inactive
scanParkAngleY = -10;     % Numeric [deg]: Optical degrees from center position for Y galvo to park at when scanning is inactive

% Optional: Scanner position feedback
deviceNameGalvoFeedback = 'PXI1Slot3';     % string identifying NI DAQ board that reads the galvo position feedback signals. Leave empty if they are on deviceNameGalvo. Cannot be the same as deviceNameAcq when using for line scanning
