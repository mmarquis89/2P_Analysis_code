...
"C:\Program Files (x86)\WinSCP\WinSCP.exe" /log="C:\Users\Wilson Lab\Documents\MATLAB\2P_code\Postprocessing-Scripts\WinSCP.log" /script="C:\Users\Wilson Lab\Documents\MATLAB\2P_code\Postprocessing-Scripts\WinSCP-script.txt" /parameter "%expDate%" "%sid%" "%parentDir%" 
...
for %%f in ("%parentDir%_Movies\*tid*.avi") do (
	echo %%f
	echo "%%~df%%~pf%%~nf.mp4"
	"C:\Program Files\Handbrake\HandBrakeCLI.exe" --preset-import-file "C:\Users\Wilson Lab\Documents\MATLAB\2P_code\Postprocessing-Scripts\BehaviorVidConversionPresets.json" -i "%%f" -o "%%~df%%~pf%%~nf.mp4" 
)
