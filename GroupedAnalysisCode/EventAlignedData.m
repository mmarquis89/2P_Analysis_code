...
% ==================================================================================================
    ...
    methods
        ...
        function obj = EventAlignedData(expList, varargin) 
            ...
            obj.sourceMd = inner_join(trialMd(:, {'expID', 'trialNum', 'nVolumes', ...
        ...
        function obj = set_align_event(obj, eventName)
            ...
            for iEvent = 1:size(alignEventData, 1)    
                ...
                [startPadVols(iEvent), endPadVols(iEvent), onsetVol(iEvent), startVol(iEvent), ...
                        endVol(iEvent)] = calculate_analysis_inds(obj.maxEventWin(1), ...
                        obj.maxEventWin(2), currEventData.onsetTime, currEventData.volumeRate, ...
                        currEventData.nVolumes);
            end
        ...
        function obj = extract_roi_data(obj)
            ...
            sourceFlData = inner_join(obj.sourceMd(:, {'expID', 'trialNum'}), roiData(:, ...
                    {'expID', 'trialNum', 'roiName', 'rawFl', 'trialBaseline', 'expBaseline'}));
        ...
        function obj = extract_fictrac_data(obj)
            ...
            if ~isempty(ftData)
                ftData = outerjoin(obj.sourceMd, ftData(:, {'expID', 'trialNum', ...
                    ...
                    onsetFrame = []; startFrame = []; endFrame = [];
                    ...
                    for iRow = 1:size(eventFt, 1)
                        ...
                        if ~isempty(ftFrameTimes)
                            ...
                            [startPadFrames(iRow), endPadFrames(iRow), onsetFrame(iRow), startFrame(iRow), ...
                                endFrame(iRow)] = calculate_analysis_inds(obj.maxEventWin(1), ...
                                obj.maxEventWin(2), currRow.onsetTime, avgFrameRate, ...
                                numel(ftFrameTimesResamp{iRow}));
                        end%if
                    end%iRow
                    ...
                    eventFtTable.eventMoveSpeed = eventMoveSpeed;
                    eventFtTable.eventYawSpeed = eventYawSpeed;
                    eventFtTable.eventFrameTimes = eventFrameTimes;
        ...
        function obj = create_filter_event_vectors(obj)
            ...
            for iObj = 1:numel(eventObjNames)
                ...
                filterEventData = innerjoin(currEventObj.eventData, obj.sourceMd(:, {'expID', 'trialNum', ...
                    'nVolumes', 'volumeRate'}));
                disp(currObjName)
                trialList = unique(filterEventData(:, {'expID', 'trialNum', 'nVolumes', 'volumeRate'}));
                
                for iTrial = 1:size(trialList, 1)
                    currTrial = trialList(iTrial, :);
                    eventArr = currEventObj.create_logical_array(currTrial.nVolumes, currTrial.volumeRate, ...
                        currTrial);
        ...
        function outputTable = output_analysis_subset(obj, analysisWin, filters)
            ...
            if ~isempty(analysisWin)
                ...
                for iRow = 1:size(dataTable, 1)
                    ...
                    % Trim volume fields
                    relVolTimes = ((1:numel(currRow.eventFl{1})) / currRow.volumeRate) - relOnsetTime;
                    ...
                    for iType = 1:numel(eventNames)
                        eventData(iRow).(eventNames{iType}) = currRow.(eventNames{iType}){1}(eventVolInds);
                    end

                    % Trim FicTrac frame fields
                    relFrameTimes = currRow.eventFrameTimes{:} - relOnsetTime;
                    eventFrameInds = relFrameTimes > -analysisWin(1) & relFrameTimes < analysisWin(2);
                    
                    frameTimes{iRow, 1} = relFrameTimes(eventFrameInds);
                    moveSpeed{iRow, 1} = currRow.eventMoveSpeed{:}(eventFrameInds);
                    yawSpeed{iRow, 1} = currRow.eventYawSpeed{:}(eventFrameInds);

                end

                outputTable = dataTable(:, {'expID', 'trialNum', 'onsetTime', 'roiName', ...
                        'trialBaseline', 'expBaseline'});
...
% Local functions
% ==================================================================================================
function [startPad, endPad, onsetSample, startSample, endSample] = calculate_analysis_inds(...
        preEventWin, postEventWin, onsetTime, sampRate, nSamples)
    
% Calculate sizes of the analysis window in samples
preEventWinSamples = ceil(preEventWin * sampRate);
postEventWinSamples = ceil(postEventWin * sampRate);

% Calculate onset, start, and end samples 
onsetSample = round(onsetTime * sampRate);
startSample = onsetSample - preEventWinSamples;
endSample = onsetSample + postEventWinSamples; 

% Calculate size of nan padding
if startSample < 1
    startPad = -startSample + 1;
    startSample = 1;
else
    startPad = 0;
end
if endSample > nSamples
    endPad = endSample - nSamples;
    endSample = nSamples;
else
    endPad = 0;
end
