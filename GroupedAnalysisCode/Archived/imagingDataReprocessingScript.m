...
for iExp = 1:size(expList, 1)
    ...
    if exist(fullfile(parentDir, expDirName), 'dir')
        ...
        if ~isempty(imgDataFiles)
            ...
            % Extract their sid and block numbers
            sidNums = str2double(regexp(fileNames, '(?<=sid_).', 'match', 'once'));
            bidNums = str2double(regexp(fileNames, '(?<=bid_).', 'match', 'once'));
            
            % Get list of each distinct block
            blockList = unique(table(sidNums, bidNums, 'VariableNames', {'sid', 'blockNum'}));
            
            blockStartTrials = 1:20:numel(imgDataFiles);
            ...
            for iBlock = 1:nBlocks
                ...
                if iBlock < nBlocks
                    ...
                    currBlockTrials = blockStartTrials(iBlock):numel(imgDataFiles);
                end
                
                % Load and concatenate imgData for current block
                for iFile = 1:numel(currBlockTrials)
                    fileName = fullfile(imgDataFiles(currBlockTrials(iFile)).folder, ...
                            imgDataFiles(currBlockTrials(iFile)).name);
                    disp(['Loading ', imgDataFiles(currBlockTrials(iFile)).name]);
                ...
                save(fullfile(parentDir, expDirName, ['refImages_reg_block_', ...
                        pad(num2str(iBlock), 3, 'left', '0'), '.mat']), 'refImages', ...
                        'regTemplates');
                ...
                saveFileName = ['imagingData_reg_block_', pad(num2str(iBlock), 3, 'left', '0'), ...
                        '.mat'];
