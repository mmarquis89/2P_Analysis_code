...
PPL201 = expMeanDff(contains(expMeanDff.roiName, 'TypeB'), :);
...
plot(ones(size(PPL201, 1), 1), PPL201.mean_dff, 'o', ...
        'markerfacecolor', rgb('green'), 'markerEdgeColor', rgb('green'));
...
%         std_err(PPL203.mean_dff)], '-s', 'color', 'k', 'markerFaceColor', 'k', 'linewidth', ...
%         3, 'markerSize', 10)
errorbar([1, 2], [mean(PPL201.mean_dff), mean(PPL203.mean_dff)], [std_err(PPL201.mean_dff), ...
        std_err(PPL203.mean_dff)], '-s', 'color', 'k', 'markerFaceColor', 'k', 'linewidth', ...
        3, 'markerSize', 10)
xlim([0.5, 2.5])
ylim([-0.001, 0.4])
...
ax.XTickLabel = {'PPL201', 'PPM203'};
ylabel('dF/F');
ax.FontSize = 14;
% [~, p] = ttest2(PPM1201.mean_dff, PPL203.mean_dff);
[~, p] = ttest2(PPL201.mean_dff, PPL203.mean_dff);
title({'Panels flash dF/F', ['p = ', num2str(p, 3)]})
...
saveDir = ['C:\Users\Wilson Lab\Documents\MATLAB\2P_code\GroupedAnalysisCode\', ...
            'Manuscript_figure_drafts\visual_responses'];
fileName = 'panels_flash_dff_comparison';
